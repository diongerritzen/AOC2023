function parseId(game) {
	return game.match(/\d+/)[0];
}
...
function parseDraws(draws) {
	return draws
		...
		.map(draw => {
			const drawnColors = {};
			for (color of draw.split(', ')) {
				drawnColors[color.split(' ')[1]] = Number(color.split(' ')[0]);
			}
			return drawnColors;
		});
}
...
function isPossibleFromBag(draw) {
	const maxCubes = {
		red: 12,
		green: 13,
		blue: 14
	};
	return Object.entries(draw).every(([key, value]) => value <= maxCubes[key])
}

function getValidGameId(line) {
	const gameData = parseLine(line);
	if (gameData.draws.every(draw => isPossibleFromBag(draw))) {
		return Number(gameData.id);
	}

	return 0;
}

function getDay2Result(input) {
	return input
		.split('\n')
		.map(line => getValidGameId(line))
		.reduce((acc, cur) => acc + cur, 0);
}

function getMinSet(line) {
	const minSet = {
		red: 0,
		green: 0,
		blue: 0
	};
	parseLine(line).draws.forEach(
		draw => Object.entries(draw).forEach(
			([key, value]) => minSet[key] = Math.max(value, minSet[key])
		)
	);
...
function getPowerOfMinSet(line) {
	return Object.values(getMinSet(line)).reduce((acc, cur) => acc * cur, 1);
}

function getDay2Part2Result(input) {
	return input
		.split('\n')
		.map(line => getPowerOfMinSet(line))
		.reduce((acc, cur) => acc + cur, 0);
}

const day2InputExample = `Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green`;

const day2Input = `Game 1: 1 red, 10 blue, 5 green; 11 blue, 6 green; 6 green; 1 green, 1 red, 12 blue; 3 blue; 3 blue, 4 green, 1 red
