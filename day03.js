...
function getPartNumbers(matrix) {
	...
	matrix.forEach((row, y) => {
		[...row.join('').matchAll(/\d+/g)].forEach(match => {
			if (isAdjacentToSymbol(match[0], match.index, y, matrix)) {
...
function isAdjacentToSymbol(num, xPos, yPos, matrix) {
	const len = num.length;
	for (let y = yPos-1; y <= yPos+1; y++) {
		if (!matrix[y]) continue;
		for (let x = xPos-1; x <= xPos+len; x++) {
			if (matrix[y][x]?.match(/[^\w\d\s.]/)) {
				return true;
			}
		}
	}
...
function returnDay3Part1Result(input) {
	return getPartNumbers(matrixifyInput(input)).reduce((acc, cur) => acc + cur, 0);
}

function findNumberWithIndex(xPos, row) {
	if (!row[xPos].match(/\d/)) return {};
	let startX = xPos;
	let endX = xPos;
	while (startX > 0 && row[startX-1].match(/\d/)) {
		startX--;
	}
	while (endX < row.length-1 && row[endX+1].match(/\d/)) {
		endX++;
	}
	return {num: Number(row.slice(startX, endX+1).join('')), index: startX};
}

function getAdjacentPartNumbers(xPos, yPos, matrix) {
	const adjPartNums = [];
	for (let y = yPos-1; y <= yPos+1; y++) {
		if (!matrix[y]) continue;
		let foundIndex = -1;
		for (let x = xPos-1; x <= xPos+1; x++) {
			if (matrix[y][x]?.match(/\d/)) {
				const {num, index} = findNumberWithIndex(x, matrix[y]);
				if (foundIndex != index) {
					adjPartNums.push(num);
					foundIndex = index;
				}
			}
		}
...
function getGearRatios(matrix) {
	const gearRatios = [];
	matrix.forEach((row, y) => {
		[...row.join('').matchAll(/\*/g)].forEach(match => {
			const adjPartNums = getAdjacentPartNumbers(match.index, y, matrix);
			if (adjPartNums.length === 2) {
				gearRatios.push(adjPartNums[0] * adjPartNums[1]);
			}
		})
	});
	return gearRatios;
...
function returnDay3Part2Result(input) {
	return getGearRatios(matrixifyInput(input)).reduce((acc, cur) => acc + cur, 0);
}
