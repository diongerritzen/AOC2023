...
function lastDigit(input) {
  return firstDigit(rev(input));
}

function replaceFirstWordWithDigit(input) {
  const digitWordsRegex = /(one)|(two)|(three)|(four)|(five)|(six)|(seven)|(eight)|(nine)/g;
  const match = [...input.matchAll(digitWordsRegex)][0];
  return match ? input.replace(match[0], match.indexOf(match[0], 1)) : input;
}

function replaceLastWordWithDigit(input) {
  const digitWordsRegex = /(eno)|(owt)|(eerht)|(ruof)|(evif)|(xis)|(neves)|(thgie)|(enin)/g;
  let reverseInput = rev(input);
  const match = [...reverseInput.matchAll(digitWordsRegex)][0];
  return match ? rev(reverseInput.replace(match[0], match.indexOf(match[0], 1))) : input;
}

function getCalibrationValue(input) {
  return Number(firstDigit(replaceFirstWordWithDigit(input)) + '' + lastDigit(replaceLastWordWithDigit(input)));
}

function getDay1Result(input) {
  return input.split('\n').map(line => getCalibrationValue(line)).reduce((acc, cur) => acc + cur, 0);
}


const day1Input = `7jlncfksix7rjgrpglmn9
